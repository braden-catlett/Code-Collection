...
function hidedrawer() {
	...
	if (window.event.clientY < divdrawer.offsetTop && drawervisible)
		handledrawer(false);
}
...
function handledrawer(visible) {
	...
	if (divdrawer != null && imgleft != null && imgright != null) {
		...
		//if (parseInt(divdrawer.style.bottom) <= drawerhide && !visible) return;
		//if (parseInt(divdrawer.style.bottom) >= drawershow && visible) return;
		...
		*/
		drawerintervalid = setInterval(function() { 
			...
			if (parseInt(divdrawer.style.bottom) < drawerhide || parseInt(divdrawer.style.bottom) > drawershow) {
...
function gettrendingimgs(fnHaveImages) {
	var xmlhttp = new XMLHttpRequest();
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			var xmlDoc = xmlhttp.responseXML;
			...
			fnHaveImages(titles, tnails);
		}
	}
	var url = "/xml/xml_gettrendinglist.php";
	xmlhttp.open("GET", url, true);
	xmlhttp.send();
}
