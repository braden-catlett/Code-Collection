...
//		webview.getSettings().setPluginState(WebSettings.PluginState.ON);
	...
	/**
	 * Sets the profile from the media carasel
	 * @param profile
	 */
	public void launchProfile(ProfileItem profile) {
			...
			new AsyncTask<Void, Void, String>()
			{
				...
				protected String doInBackground(Void... params)
				{
					...
					{						
						...
						postParameters.add(new Pair("id", selectedVideoItem.ID));
//						postParameters.add(new Pair("click", "1"));
//						postParameters.add(new Pair("share", "0"));
						postParameters.add(new Pair("header", "0"));

						BufferedInputStream resp = WebUtil.GetHTTPSResponse(showvideoURL, postParameters, getActivity());
						if(resp != null) {
							...
							resp.close();
							Log.e("SHOWVIDEO", "Got Response");
						}
						else {
							Log.e("SHOWVIDEO", "No Response");
						}
					}
					catch (Exception ex)
					{
...
//						DisplayMetrics m = getSherlockActivity().getResources().getDisplayMetrics();
						...
						getSherlockActivity().getWindowManager().getDefaultDisplay().getMetrics(m);
//						int height = m.heightPixels;
						int width = m.widthPixels;
						int height = (getView().findViewById(R.id.videowebview)).getHeight();
//						if(orientation == Configuration.ORIENTATION_PORTRAIT)
//							height += 100;
//						int width = (int) (getView().findViewById(R.id.videowebview)).getWidth();
...
//						}
	...
	private class VideoAsyncTask extends AsyncTask<Void, ArrayList<LinearLayout>, Void>
	{
		...
		public ArrayList<LinearLayout> populateVideoList()
		{
			...
			try
			{
				postParameters.add(new Pair("ProfID", selectedProfileItem.pid));
