...
public class Chat {
	...
	//Array Lists
		...
		defaultuser = new user("Guest","",true);
		me = defaultuser;
	...
	private void update() {
		...
		for(user u : active) {
				...
				System.out.println("appending " + u.getUsername());
		}
	}
		...
		public void run() {
			...
			try {				
				while((message = in.readObject()) != null) {
					...
					//This check is to prevent a bug when you logout and and the program reads in a message keeping the "main" frame visible
					if( me.getUsername() == "" ) {
						continue;
					}
					...
					if( message instanceof user ) {
						if(active.contains((user) message) == false) {
							active.add((user) message);
						}
					}
